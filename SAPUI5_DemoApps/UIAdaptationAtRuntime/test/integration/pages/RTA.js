...
sap.ui.define([
	...
	var interactableMatcher = new Interactable();
		...
		onTheMasterPageWithRTA: {
			...
			actions: {
				...
				iPressOnAdaptUi: function(bPersonalize) {
					...
					return this.waitFor({
						id: 'overflowActions-overflowButton',
						errorMessage: "Did not find the More button in Me-Area",
						actions: new Press(),
						success: function () {
							return this.waitFor({
								id: sButtonType + "_Plugin_ActionButton",
				...
				iPressCancel: function() {
					...
					return this.waitFor({
						...
						matchers: new sap.ui.test.matchers.PropertyStrictEquals({
							name: "text",
							value: oResources.getText("MSG_PERSONALIZATION_CANCEL_BUTTON_TEXT")
						}),
						actions: new Press(),
				...
				iExitRtaMode: function() {
					...
					return this.waitFor({
						controlType: "sap.m.Button",
						matchers: function(oButton) {
							return oButton.getParent().$().hasClass("sapUiRtaToolbar") && oButton.getProperty("text") === oResources.getText("BTN_EXIT");
						},
						actions: new Press()
					})
				},
				...
				iPressOnAddSection: function(sSectionId) {
					return this.waitFor({
						...
						success: function(aOverlays){
							var oOverlay = aOverlays[0];
							var sOverlayId = oOverlay.getId();
							oOverlay.$().find("#" + sOverlayId + "-AddButton").trigger("tap");
						},
			...
			assertions: {
				...
				iShouldSeeTheFLPToolbarAndChangesInLRep: function(iCount) {
					return this.waitFor({
						...
						success: function(oToolbar) {
							...
							assert.equal(FakeLrepLocalStorage.getNumChanges(), iCount, "the number of changes is correct");
						},
				...
				theGroupElementHasTheLastIndex: function(sId) {
					return this.waitFor({
						controlType: "sap.ui.dt.ElementOverlay",
						matchers: function(oOverlay) {
							return oOverlay.getElementInstance().getId() === sId
						...
						success: function(aOverlays) {
							var oOverlay = aOverlays[0];
							var aGroupElements = oOverlay.getParentElementOverlay().getElementInstance().getGroupElements();
							assert.equal(aGroupElements[aGroupElements.length - 1].getId(), sId, "the Group Element has the correct Index");
						},
						errorMessage: "Did not find the Element Overlay"
					});
				},
				...
				theChangesToTheGroupShouldStillBeThere: function(sGroupId, sElementId1, sElementId2, iCount) {
					return this.waitFor({
						...
						success: function(oGroup) {
							...
							assert.equal(aFormElements[aFormElements.length - 1].getId(), sElementId1, "then the last Element is correct");
				...
				theSectionShouldBeInTheFirstPosition: function(sId) {
					return this.waitFor({
						...
						success: function(aOverlays) {
							var oOverlay = aOverlays[0];
							var aSections = oOverlay.getParentElementOverlay().getElementInstance().getSections();
